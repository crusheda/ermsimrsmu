...
class JadwalController extends Controller
{
    ...
    public function totalAbsensi($pegawaiId, $range)
    {
        ...
        $currentStart = $now->day >= 21
            ? $now->copy()->day(21)
            : $now->copy()->subMonthNoOverflow()->day(21);

        $currentEnd = $currentStart->copy()->addMonthNoOverflow()->day(20);

        $offset = (int)(1 - $range);

        $startDate = $currentStart->copy()->subMonthsNoOverflow($offset);
        $endDate   = $currentEnd->copy()->subMonthsNoOverflow($offset);

        $maxHari   = 31;
        $kodeLibur = ['L','C','CM','CU','CH','CD'];

        // ===== 1) TOTAL ABSENSI =====
        $totalAbsensi = DB::table('kepegawaian_absensi')
            ->where('pegawai_id', $pegawaiId)
            ->where('jenis', 1)
            ->whereNull('deleted_at')
        ...
        // ===== 2) SUSUN LIST BULAN YANG TERLIBAT =====
        $months = [
            [
                'bulan' => (int)$startDate->format('m'),
        ...
        if ($startDate->format('Ym') !== $endDate->format('Ym')) {
            $months[] = [
                'bulan' => (int)$endDate->format('m'),
                'tahun' => (int)$endDate->format('Y')
            ];
        }


        // ===== 3) QUERY JADWAL (pakai where ->orWhere) =====
        $jadwalRows = DB::table('kepegawaian_jadwal_detail as jd')
            ->join('kepegawaian_jadwal as j', function ($join) use ($months) {

                $join->on('j.id', '=', 'jd.id_jadwal')
                    ->whereIn('j.progress', [1,2,3])
                    ->whereNull('j.deleted_at');

                $join->where(function ($q) use ($months) {
                    foreach ($months as $m) {
                        $q->orWhere(function ($x) use ($m) {
                            $x->where('j.bulan', $m['bulan'])
                            ->where('j.tahun', $m['tahun']);
                        });
                    }
                });
            })
        ...
        while ($cursor->lte($endDate)) {
            ...
            if (isset($jadwalMap[$key])) {
                ...
                $kolom = 'tgl'.$cursor->day;
                $kode  = $jadwalMap[$key]->$kolom ?? null;

                if ($kode && !in_array($kode, $kodeLibur)) {
                    $totalHariKerja++;
                }
            }
    ...
    function tambahShift(Request $request)
    {
        ...
        // Jika pulang lebih kecil dari berangkat, tambahkan 1 hari ke pulang (shift malam)
    ...
    function hapusShift($id)
    {
        ...
        if (!$data) {
            return response()->json(['message' => 'Data shift tidak ditemukan / sudah terhapus. Silakan cek kembali shift Anda.'], 500);
        }

        // Hitung selisih bulan antara created_at dan sekarang
        $selisihBulan = Carbon::parse($data->created_at)->diffInMonths(Carbon::now());

        // Jika selisih < 3 bulan, larang hapus
        if ($selisihBulan < 3) {
            return response()->json(['message' => 'Shift tidak dapat dihapus karena belum melewati 3 bulan sejak dibuat ('.$data->created_at->format('d-m-Y').'). Silakan Hubungi Admin untuk penghapusan lebih lanjut.'], 500);
        }

        // Jika sudah lebih dari 3 bulan, boleh hapus
        $data->delete();

        return response()->json([
            'message' => 'Shift berhasil dihapus pada '.$tgl,
            'code' => 200,
        ],200);
    }
    ...
    function ambilAlihStaf($id,$user)
    {
        ...
        if ($cek) {
            ...
            $data3 = ref_jadwal_shift::withTrashed()->where('pegawai_id', $id)->get();
            if ($data3->count() > 0) {
