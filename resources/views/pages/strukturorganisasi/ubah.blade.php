...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Struktur Organisasi - Ubah <kbd>ID : {{ $list['show']->id }}</kbd></h4>
            </div>
        </div>
    </div>
    ...
    <div class="card">
        ...
        {{-- <select class="selectpicker form-control mt-4" data-live-search="true" data-actions-box="true" multiple title="Choose one of the following..." data-header="Select a condiment" data-style="btn-light">
            ...
            {{ Form::model($list['show'], ['route' => ['strukturorganisasi.update', $list['show']->id], 'method' => 'PUT', 'id' => 'formUbah']) }}
            @csrf
            <div class="form-group mb-4">
                <label for="userx" class="form-label">Nama User</label>
                <select class="selectpicker form-control mt-4" id="userx" name="user" data-width="100%" data-live-search="true" data-actions-box="true" multiple title="Choose one of the following..." data-header="Pilih Nama Karyawan" data-style="btn-light">
                {{-- <select id="userx" name="user" class="form-control form-select select2" style="width: 100%" required data-placeholder=""> --}}
                    @if (count($list['user']) > 0)
                        @foreach ($list['user'] as $item)
                            {{-- <option>{{ $item->nama }}</option> --}}
                            <option value="{{ $item->id }}" @if ($item->id == $list['show']->id_user) selected @endif>{{ $item->nama }}</option>
                        @endforeach
                    @endif
                </select>
